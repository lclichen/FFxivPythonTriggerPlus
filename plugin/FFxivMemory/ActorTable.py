...
from .models.Actor import ActorTable
from .models.MemoryParseObject import get_memory_array, get_pointer
from time import time

_pattern = b"\x48\x8D.....\xE8....\x48\x8B.\x48\x8B.\x48\x8D.....\xE8....\x48\x8D.....\xBA....\xE8....\x89\x2F"
sig = "48 8d ?? ?? ?? ?? ?? e8 ?? ?? ?? ?? 48 8b ?? 48 8b ?? 48 8d ?? ?? ?? ?? ?? " \
      "e8 ?? ?? ?? ?? 48 8d ?? ?? ?? ?? ?? ba ?? ?? ?? ?? e8 ?? ?? ?? ?? 89 2f"
auto_update_sec = 0.5
Length = 200


def get_actor_table(handler: MemoryHandler):
    pattern = handler.ida_sig_to_pattern(sig)
    addr = handler.scan_pointer_by_pattern(pattern, 7)
    return ActorTable(handler, addr)
