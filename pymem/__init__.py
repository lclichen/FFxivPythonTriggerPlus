...
class Pymem(object):
    ...
    def inject_python_interpreter(self, initsigs=1):
        ...
        def find_existing_interpreter(_python_version):
            ...
            self.commands[b'PyRun_SimpleString'] = (
                    module.lpBaseOfDll + (
            ...
            pymem.logger.debug('PyRun_SimpleString loc: 0x%08x' % self.commands[b'PyRun_SimpleString'])
        ...
        for command in [b'Py_InitializeEx', b'PyRun_SimpleString',b'Py_SetPath',b'PyRun_SimpleFile']:
            self.commands[command] = (
                    python_lib_h + (
                    pymem.ressources.kernel32.GetProcAddress(local_handle, command)
                    - local_handle
            )
            )
            if not self.commands[command]:
                raise pymem.exception.PymemError('%s not found' % command)

        param_addr = self.allocate(4)
        self.write_int(param_addr, initsigs)
        self.start_thread(self.commands[b'Py_InitializeEx'], param_addr)

        self._python_injected = True

        pymem.logger.debug('Py_InitializeEx loc: 0x%08x' % self.commands[b'Py_InitializeEx'])
        pymem.logger.debug('PyRun_SimpleString loc: 0x%08x' %self.commands[b'PyRun_SimpleString'])
    ...
    def inject_python_shellcode(self, shellcode):
        ...
        # check written
        self.start_thread(self.commands[b'PyRun_SimpleString'], shellcode_addr)
